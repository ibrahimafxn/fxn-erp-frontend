...
import { Depot, Material, Movement, MovementListResult, User } from '../../../core/models';
...
@Component({
  ...
  templateUrl: './material-reservations-list.html',
  styleUrl: './material-reservations-list.scss',
})
export class MaterialReservationsList {
  ...
  private readonly currentMaterialDepot = signal<string | null>(null);

  constructor() {
    this.loadUsers();
    this.loadDepots();
    this.loadMaterials(this.isDepotManager() ? this.managerDepotId() : null);
    this.refresh(true);
  }
  ...
  releaseFromRow(m: Movement): void {
    ...
    this.materialService.releaseReservation({
      materialId: m.resourceId,
  ...
  private loadMaterials(depotId: string | null): void {
    if (depotId && this.currentMaterialDepot() === depotId && this.materials().length > 0) return;
    if (!depotId && this.currentMaterialDepot() === null && this.materials().length > 0) return;

    this.currentMaterialDepot.set(depotId);
